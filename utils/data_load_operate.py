...
def load_data(data_set_name, data_path='./data'):
    ...
    elif data_set_name == 'blueberry':
        data = np.load("/mnt/storage/Yimin/Posterior_sampling_unmixing_data/Hyperspectral_unmxing_dataset/cube_3/data_cube_3_0.npy")
        labels_orignial = np.load("/mnt/storage/Yimin/Posterior_sampling_unmixing_data/Hyperspectral_unmxing_dataset/cube_3/class_matrix.npy")
        H_new, W_new, _ = data.shape
        block = 3

        labels_crop = labels_orignial[:H_new * block, :W_new * block]
        labels_block = labels_crop.reshape(H_new, block, W_new, block).transpose(0, 2, 1, 3).reshape(H_new, W_new, -1)
        def majority_vote(block_vals):
            uniq, counts = np.unique(block_vals, return_counts=True)
            return uniq[np.argmax(counts)]
        # file = h5py.File("/mnt/storage/benchmark_datasets/hyperspectral/SZUTreeData2.0/SZUTreeData_R1_2.0/SZUTree_HSI_R1/data_band98.mat","r")
        # f2 = sio.loadmat("/mnt/storage/benchmark_datasets/hyperspectral/SZUTreeData2.0/SZUTreeData_R1_2.0/Annotations_SZUTreeData_R1/SZUTreeData_R1_treeid_with_labels_5cm.mat")
        # pdb.set_trace()
        labels = np.apply_along_axis(majority_vote, axis=2, arr=labels_block) + 1

        print(labels.shape)  # (341, 938)
    elif data_set_name == "QUH":
        data = sio.loadmat("/s3/benchmark_datasets/hyperspectral/QUH-Tangdaowan.mat")["Tangdaowan"]  # 1740 × 860 × 176
        labels = sio.loadmat("/s3/benchmark_datasets/hyperspectral/QUH-Tangdaowan_GT.mat")['TangdaowanGT']
        
    elif data_set_name == "YYC":
        # data = sio.loadmat("/mnt/storage/benchmark_datasets/hyperspectral/ZY_YC_data147.mat")['Data']
        # labels = sio.loadmat("/mnt/storage/benchmark_datasets/hyperspectral/ZY_YC_gt7.mat")['DataClass']
        # pdb.set_trace()
        data = sio.loadmat("/s3/benchmark_datasets/hyperspectral/GF_YC_data.mat")['Data']
        labels = sio.loadmat("/s3/benchmark_datasets/hyperspectral/GF_YC_gt.mat")['DataClass']
        

    elif data_set_name == "Berlin":
        H, W = 1723, 476 
        data = sio.loadmat("/mnt/storage/benchmark_datasets/hyperspectral/Berlin_edata.mat")['Y'].reshape(-1, W, H).transpose(1,2,0)
        labels = sio.loadmat("/mnt/storage/benchmark_datasets/hyperspectral/Berlin_gt.mat")['gt'].transpose(1,0)

    elif data_set_name == "UAV":
        data = sio.loadmat("/mnt/storage/benchmark_datasets/hyperspectral/UAVHer.mat")['image']
        labels = sio.loadmat("/mnt/storage/benchmark_datasets/hyperspectral/UAVHer_gt.mat")['label']

    elif data_set_name == "HU":
        H, W = 1905, 349
        data = sio.loadmat("/mnt/storage/benchmark_datasets/hyperspectral/Houston_em.mat")['Y']
        C = data.shape[0]
        data = data.reshape(C, H, W).transpose(2, 1, 0)
        labels = sio.loadmat("/mnt/storage/benchmark_datasets/hyperspectral/Houston_gt.mat")['Houston_gt']
    elif data_set_name == 'Tea':
        img = open_image("/s3/benchmark_datasets/hyperspectral/PHI_FangluTeaFarm.hdr")   # point to .hdr
        input = img.load()    
        data = np.array(input) # usually returns [H, W, B]
        gt_path = "/s3/benchmark_datasets/hyperspectral/PHI_GroundTruthFanglu.img"
        gt = np.fromfile(gt_path, dtype=np.uint8)
        gt = gt.reshape(1, 512, 348)   # BSQ: (bands, lines, samples)
        labels = gt[0]  
...
def sampling(ratio_list, num_list, gt_reshape, class_count, Flag):
    ...
    val_num = [3, 41, 24, 6, 14, 21, 3, 13, 3, 28, 70, 17, 6, 37, 11, 3]
